...
namespace lab10New
{
    class Program
    {
        ...
        {
            using (TickerDb db = new TickerDb())
            {
            ...
            while (tickerName != "exit")
            {
                using (TickerDb db = new TickerDb())
                {
                    ...
                    foreach (var us in users)
                        Console.WriteLine($"{us.Id} ({us.Ticker.Ticker1}) - {us.Price1}");
        ...
        {
            string path = @"C:\Users\artem\source\repos\lab09\ticker.txt";
            int IdTicker = 1;
            int IdPrice = 1;
            using (TickerDb db = new TickerDb())
            {
        ...
        static bool AddPrice(TickerDb db, int tickerID, string ticker, float price, DateTime dateTime)
        {
                var newTicker = new Ticker()
                {
                    Id = tickerID,
        ...
        static bool AddTicker(TickerDb db, int tickerID, string ticker)
        {
                var newTicker = new Ticker()
                {
                    Id = tickerID,
